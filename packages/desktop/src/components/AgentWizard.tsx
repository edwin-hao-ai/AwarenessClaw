...
export default function AgentWizard({ onComplete, onCancel }: AgentWizardProps) {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      const slug = finalName.toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '') || `agent-${Date.now()}`;
        ...
        <div className="bg-slate-900/80 rounded-2xl border border-slate-800 p-6 min-h-[300px] flex flex-col">
          ...
          {step === 0 && (
            <div className="flex-1 flex flex-col gap-4">
              ...
              <p className="text-[10px] text-slate-600 text-center">{t('agentWizard.step1.naming', 'Letters, numbers, and hyphens only')}</p>
            </div>
          )}
